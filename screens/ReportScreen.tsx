...
import { DerogaCode, DerogaItem } from '../types';
...
export default function ReportScreen() {
  ...
  const grouped = useMemo(() => {
    const byInf: Record<string, { nome: string; items: DerogaItem[] }> = {};
    deroghe.forEach((d) => {
      if (!byInf[d.infId]) byInf[d.infId] = { nome: d.nome, items: [] };
      byInf[d.infId].items.push(d);
    });
    return byInf;
  }, [deroghe]);
    ...
    <SafeAreaView edges={['top']} style={[styles.safe, { backgroundColor: colors.bg }]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Button
          ...
          variant="soft"
          full
          icon={<Ionicons name="grid-outline" size={18} color={colors.blue} />}
